...
rm -rf ${APP_DEST} && \
mkdir ${APP_DEST} && \
rsync -a --exclude .git* --exclude .gitlab-ci* --exclude .github --exclude screenshot* --exclude docs --exclude tests --exclude vendor --exclude package.* --exclude Makefile --exclude *.db* --exclude docker --exclude *.phar --exclude *.gz --exclude node_modules --exclude .idea ${APP_SOURCE}/ ${APP_DEST} && \
