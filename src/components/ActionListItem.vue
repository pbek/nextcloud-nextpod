<template>
  <div>
    <div @click.self="showModalEpisodeDescription" style="cursor: pointer">
      <NcListItem
        :name="isLoading ? action.episodeUrl : getEpisodeName()"
        :details="getDetails()"
        :force-display-actions="true"
        @click.native="onListItemClick"
      >
        <template #icon>
          <!--
...
export default {
  ...
  methods: {
    onListItemClick(event) {
      console.log("onListItemClick called", event.target);
      // Check if the click is on the actions menu, actions button, or their children
      const target = event.target;
      const actionsMenu = target.closest(".list-item__actions");
      const actionsButton =
      ...
      // Only open dialog if not clicking on actions menu or button
      if (!actionsMenu && !actionsButton) {
        console.log("Calling showModalEpisodeDescription");
        this.showModalEpisodeDescription();
      }
    },
    playAudio() {
      ...
      if (!this.playTimeSet) {
        if (
          this.action.position !== this.action.total &&
          this.action.position !== -1
        ) {
    ...
    async loadActionExtraData() {
      try {
        const resp = await axios.get(
          generateUrl(
            "/apps/nextpod/personal_settings/action_extra_data?episodeUrl={url}",
            {
    ...
    showModalPlayer() {
      console.log("showModalPlayer called");
      this.playTimeSet = false;
      this.modalPlayer = true;
      console.log("modalPlayer set to:", this.modalPlayer);
    },
    closeModalPlayer() {
      this.modalPlayer = false;
    },
    showModalEpisodeDescription() {
      console.log("showModalEpisodeDescription called");
      this.modalEpisodeDescription = true;
      console.log(
        "modalEpisodeDescription set to:",
        this.modalEpisodeDescription,
      );
    },
    ...
    async postNote() {
      ...
      const resp = await axios
        ...
        .then(function (response) {
          ...
          window
            .open(
              generateUrl("/apps/notes/note/{id}", {
                id: noteId,
              }),
              "_blank",
            )
